...
import static controllers.Controller.addresse;
import static controllers.Controller.lieu;
import static controllers.Controller.magasin;
import static controllers.Controller.telephone;
...
public class printStocks extends Controller{
 
    private String RESULT;
    public static final int numberColumn = 6, numberElement = 12;
    private int taille;
    private String str;
    
    private ArrayList<GestionStock> print;
    ...
    public ArrayList<GestionStock> getPrint() {
        return print;
    }

    public void setPrint(ArrayList<GestionStock> print) {
        this.print = print;
    }
    ...
    private void affichage(Document document, FontSelector selector, String str, int k) throws DocumentException, BadElementException, IOException{
        ...
        if(k*numberElement < taille){
            ...
            table.addCell(cell);
            cell = new PdfPCell(new Phrase("Gestionnaire"));
            table.addCell(cell);
            cell = new PdfPCell();
            cell.setRowspan(2);
        }
        ...
        for(int i = k*numberElement; i < k*numberElement + numberElement; i++){
            for(int aw = 0; aw < numberColumn; aw++){
                if(aw % numberColumn == 0){
                    if(i < taille) cell.setPhrase(new Phrase(print.get(i).getIdCodeString()));
                }
                else if(aw % numberColumn == 1){
                    if(i < taille) cell.setPhrase(new Phrase(print.get(i).getNomproduit()));
                }
                else if(aw % numberColumn == 2){
                    if(i < taille) cell.setPhrase(new Phrase(print.get(i).getQuantite()+""));
                }
                else if(aw % numberColumn == 3){
                    if(i < taille) cell.setPhrase(new Phrase(print.get(i).getDatestock()));
                }
                else if(aw % numberColumn == 4){
                    if(i < taille) cell.setPhrase(new Phrase(print.get(i).getTypeString()));
                }
                else {
                    if(i < taille) cell.setPhrase(new Phrase(print.get(i).getNomGestString()));
                }
